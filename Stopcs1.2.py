...
for x in parsed_json['component']['referencingComponents']:
    ...
    ref_comp_id=x['id']
    payload={"id":cs_id,"state":"STOPPED","referencingComponentRevisions":{ref_comp_id:{"version":ver}}}
    json_payload=json.dumps(payload)
    resp=requests.put(ref_url, data=json_payload, headers=headers)
    print(resp.content)
    time.sleep(10)
#Disable references
discsref_payload={"id":cs_id,"state":"DISABLED","referencingComponentRevisions":{}}
json_dis_payload=json.dumps(discsref_payload)
...
discs_payload={"revision":{"version":vercs},"component":{"id":cs_id,"state":"DISABLED"}}
